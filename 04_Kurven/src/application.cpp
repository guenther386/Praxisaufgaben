...
#define STEP_GRANUALIRITY 0.005
...
void application::context_menu_select(int item)
{
	...
	case MA_SHOW_GRID:
		...
		menu_dirty = true;
		break;
	case MA_INCREASE_CASTELJAU_T:
		if (dynamic_cast<bezier_curve*>(curve)) {
			curve->casteljau_t = (curve->casteljau_t + STEP_GRANUALIRITY, 1.0);
		}
		break;
	case MA_DECREASE_CASTELJAU_T:
		if (dynamic_cast<bezier_curve*>(curve)) {
			curve->casteljau_t = (curve->casteljau_t - STEP_GRANUALIRITY, 0.0);
		}
		break;
	}
...
void application::setup_context_menu()
{
	...
	glutAddMenuEntry("    Increase Casteljau t        (o) ", MA_INCREASE_CASTELJAU_T);
	glutAddMenuEntry("    Decrease Casteljau t        (p) ", MA_DECREASE_CASTELJAU_T);
...
void application::key_down(unsigned char key, int x, int y)
{
	...
	case 'o': context_menu_select(MA_INCREASE_CASTELJAU_T); break;
	case 'p': context_menu_select(MA_DECREASE_CASTELJAU_T); break;
