...
tiny_vec<float, 3> intersection_info::get_direction_to_camera() const 
{
	return -1.0f * incoming_ray.get_direction();
}
...
tiny_vec<float,3> intersection_info::get_direction_to_light() const
{
	return light_dir;
}

void intersection_info::set_direction_to_light(const tiny_vec<float,3>& l) 
{
	light_dir=l;
}
...
tiny_vec<float,3> intersection_info::get_reflected_view_direction() const 
{
...
tiny_vec<float,3> intersection_info::get_refracted_view_direction(float n1, float n2,bool &total_reflection) const 
{
	tiny_vec<float,3> V =  incoming_ray.get_direction();
	float n = n1 / n2;
	float cosI = dot(normal, V);
