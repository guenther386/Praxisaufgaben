...
box::box(const tiny_vec<float,3>& minimum, const tiny_vec<float,3>& maximum) : minimum(minimum),maximum(maximum) 
{
}
...
void box::extend(const std::pair<tiny_vec<float,3>,tiny_vec<float,3> >& minmax)
{
	extend(minmax.first);
...
bool box::closest_intersection(intersection_info* hit, float min_lambda,primitive* dont_hit)
{
	if(dont_hit == this)
		return false;
