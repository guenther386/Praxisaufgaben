...
bool kd_tree::add_obj_mesh(const std::string& filename, bool scale)
 {
...
bool kd_tree::closest_intersection(intersection_info* hit,float min_lambda,primitive* dont_hit)
{
    ...
    while( current != NULL )
    {
        ...
        for( int i=0; i<(int)((kd_leaf*)current)->get_prim_list_size(); i++ )
        {
			...
			bool bt = prim[i]->closest_intersection(hit,stack[entry].lambda,dont_hit);
			if(bt)
				hitx = bt;
 ...
 bool kd_tree::any_intersection(ray<float>& r,float min_lambda, float max_lambda,primitive* dont_hit)
{
...
// check if ray can hit the scene at all
    ...
    while( current != NULL )
    {
        ...
        for( int i=0; i<(int)((kd_leaf*)current)->get_prim_list_size(); i++ )
        {
			if(prim[i]->any_intersection(r,min_lambda,max_lambda,dont_hit))
				return true;
