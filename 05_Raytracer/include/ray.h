...
public:
	...
	{
		vec<float> p = m*homog(origin);
		set_origin(unhomog(tiny_vec<float,4>(p.begin(),p.end())));
		
		tiny_vec<float,4> dirh = homog_dir(direction);
		vec<float> d = m*dirh;
		tiny_vec<float,3> dir(d.begin(),d.end()-1);
		set_direction(dir);
	}
